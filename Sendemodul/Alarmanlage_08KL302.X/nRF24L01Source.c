...
// Should be called in the early initializing phase at startup.
{
    ...
    TRISAbits.TRISA2 = 0;                //RA2 as Output  CE
    TRISBbits.TRISB15 = 0;               //RB15 as Output  CSN=SS1
    TRISAbits.TRISA6=1;                  //RA6 as input for Interrupt2
    ...
    IEC0bits.INT0IE=1;                  //Interrupt 0  activated
    IPC0bits.INT0IP0= 7;                //set int0 priority to highest
    INTCON2bits.INT0EP=1;               //0 = Interrupt on positive edge
...
extern void wl_module_tx_config(unsigned char tx_nr)
{
    ...
    // Set RF channel
    wl_module_config_register(RF_CH,wl_module_CH);               // Set data speed & Output Power configured in wl_module.h Register 0x05 is the RF_CH register, which allows you to set your RF channel

	wl_module_config_register(RF_SETUP,wl_module_RF_SETUP); //Config the CONFIG Register (Mask IRQ, CRC, etc)

	wl_module_config_register(CONFIG, wl_module_CONFIG);    // Set length of incoming payloadwl_module_config_register(RX_PW_P0, wl_module_PAYLOAD);
...
// amount of bytes as configured as payload on the receiver.
   {   while (PTX) {}                          // Wait until last paket is send
    ...
    wl_module_CE_lo;                        //Chip Enable Activates RX or TX mode
    ...
    spi_transmit_sync(value,len);           // Write payload transfer to SSP1BUF  Testen without len
    wl_module_CSN_hi;                       // Pull up chip select
   _delay_10ms(500);
